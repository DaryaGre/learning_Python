...
profit_list = [el for el in firm_dict.values() if el > 0]

average = dict(average_profit=sum(profit_list) / len(profit_list))

print([firm_dict, average])

with open("text_7.json", "w", encoding='UTF-8') as write_f:
    json.dump([firm_dict, average], write_f, indent=4, ensure_ascii=False)
